...
class study_comet:
    ...
    def lock_on_phase_1(self,image_names):
        ...
        for image_name in tqdm(image_names):
            ...
            #comet_pic.img.data=comet_pic.img.data/comet_pic.img.exptime
            ...
            #mark_target([len(comet_pic.cutout)/2,len(comet_pic.cutout)/2],comet_pic.cutout)
        ...
        #mark_target([len(cutout_stack)/2,len(cutout_stack)/2],cutout_stack)
        
        self.comet_error = lock_comet(cutout_stack,fwhm=4,cutoff=self.comet_pixel_max)
    ...
    def mark2_comet_track(self):
        jpl_points=[]
        times=[]
        rates=[]
        for pic in self.comet_pics:
            #pix_coord = coords.SkyCoord.from_pixel(pic.orig_comet_pix_location[0],pic.orig_comet_pix_location[1],pic.img.wcs)
            jpl_coord = coords.SkyCoord(ra=pic.tgt_RA,dec=pic.tgt_DEC,unit="deg",frame="fk5")
            jpl_points.append(np.array([jpl_coord.ra.to(u.deg).value,jpl_coord.dec.to(u.deg).value]).flatten())
            times.append(pic.date)

        jpl_points=np.array(jpl_points)
        times = np.array(times)
        times = (times - times[0]) * 24 * 60 * 60
        difs = (jpl_points[1:] - jpl_points[:-1])
        dts = (times[1:]-times[:-1])
        for dif,time in zip(difs,dts):
            rates.append(dif/time)

        rates = np.array(rates)
        #Using JPL Eph data, calculate a median deg/sec rate for RA/Dec
        rate = np.median(rates,axis=0)

        ref_points = self.coord_sextractor(self.comet_pics[0])
        predict_points = np.copy(ref_points)

        shifted_coords = []
        
        
        for i in range(1,len(self.comet_pics)):
            pic = self.comet_pics[i]

            predict_points[:,0] += rate[0] * dts[i-1]
            predict_points[:,1] += rate[1] * dts[i-1]

            
            #pix_past = np.array(coords.SkyCoord(ra=points[:,0],dec=points[:,1],unit="deg",frame="fk5").to_pixel(next_pic.img.wcs)).transpose()
            pix_predict = np.array(coords.SkyCoord(ra=predict_points[:,0],dec=predict_points[:,1],unit="deg",frame="fk5").to_pixel(pic.img.wcs)).transpose()
            shifted_pixs = []
            for pix in pix_predict:
                
                new_pix,fwhm = local_peak(pic.img.data,pix,15,len(pic.img.data[:,0]),len(pic.img.data[0,:]),bkg_remove=True)
                new_pix=np.append(new_pix,fwhm)
                shifted_pixs.append(new_pix)
                
                """
                result=fit_gauss(pic.img.data,fix_fwhm=False,xypos=pix).results
                star_pos=np.array([result["x_fit"][0],result["y_fit"][0]])
                fwhm_fit=result["fwhm_fit"][0]

                shifted_pixs.append(star_pos)
                """
            shifted_pixs = np.array(shifted_pixs)
            new_sky_coords=(coords.SkyCoord.from_pixel(shifted_pixs[:,0],shifted_pixs[:,1],pic.img.wcs))
                        
            shifted_coords.append(np.column_stack((new_sky_coords.ra.to(u.deg).value,new_sky_coords.dec.to(u.deg).value,shifted_pixs[:,2])))
        shifted_coords = np.array(shifted_coords)
        #print(np.shape(shifted_coords))

        scores = []

        for i in range(0,len(shifted_coords[0,:,1])):
            fwhm_med = np.median(shifted_coords[:,i,2])
            fwhm_stddev = np.std((shifted_coords[:,i,2])-fwhm_med)

            dra = shifted_coords[:,i,0]-jpl_points[1:,0]
            dra_stddev = np.std(dra-np.median(dra))
            
            ddec = shifted_coords[:,i,1]-jpl_points[1:,1]
            ddec_stddev = np.std(ddec-np.median(ddec))

            scores.append(fwhm_stddev*dra_stddev*ddec_stddev)

            #plt.scatter(np.linspace(0,len((shifted_coords[:,i,2])),num=len((shifted_coords[:,i,2]))),(shifted_coords[:,i,2])-med)
            #plt.show()
        
        for i in range(0,len(shifted_coords[0,:,1])):
            plt.scatter(times[1:],shifted_coords[:,i,0],label=scores[i])
        plt.legend()
        plt.show()

        best_index = np.argmin(scores)
        locked_coords = shifted_coords[:,best_index,:2]

        for pic,coord in zip(self.comet_pics,locked_coords):
            pic.comet_pix_location = np.array(coords.SkyCoord(ra=coord[0],dec=coord[1],unit="deg",frame="fk5").to_pixel(pic.img.wcs)).transpose()
