...
image_path = pathlib.Path(calib_path / "149P_R#642_54859.07496021.fits") 
...
for pairs in fringe_coords:
    ...
    print("MAP DIF: ",map_dif)
    print("FRAME DIF: ",frame_dif)
    print("-----")
...
for x,y,ratio in zip(xpoints,ypoints,ratios):
    plt.text(x,y,"%.3f" % ratio)

scale=np.median(ratios)
print(scale)

#med=np.nanmedian(fringe.data)
fringe.data[fringe.mask==1] = np.nanmedian(fringe.data)
reduced_img=data.data-(fringe.data*scale)
#reduced_img[data.mask==1]=np.nan


plt.imshow((data.data),origin="lower",cmap="grey",norm=LogNorm())
plt.show()

plt.hist(ratios-scale)
plt.show()
